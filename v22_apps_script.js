/**
 * ============================================================================
 * AMAR FOODS BUYER CRM - FULL 2-WAY SYNC + FOB & CIF EXPORT CALCULATOR (V22)
 * Handles:
 ...
 *  10. FOB, CFR & CIF Export Calculator (FOB_CIF_Calculator tab) - Pipavav Port
 *  11. setupFobCifCalculatorSheet() - Builds formatted FOB/CFR/CIF Calculator in Sheet
 * ============================================================================
 */
  ...
  try {
    ...
    if (action === "updatePrices" && e.parameter.payload) {
      ...
      return jsonResponse({ status: "success", result: saveWeeklyPrices(pricePayload) });
        ...
        // ─── 5. FOB, CFR & CIF EXPORT CALCULATOR ───────────────────────────
    if (action === "setupFobCifCalculator") {
      var calcRes = setupFobCifCalculatorSheet();
      return jsonResponse({ status: "success", result: calcRes });
    }
    if (action === "getFobCifCalculator") {
      return jsonResponse({ status: "success", data: getFobCifCalculatorData() });
    }
    if (action === "updateFobCifInputs" && e.parameter) {
      var updated = updateFobCifInputs(e.parameter);
      return jsonResponse({ status: "success", result: updated });
    }
...
function doPost(e) {
  try {
    ...
    if (action === "updateFobCifInputs") {
      var updated = updateFobCifInputs(contents);
      return jsonResponse({ status: "success", result: updated });
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// 🧮 FOB, CFR & CIF EXPORT PRICE CALCULATOR (Pipavav Port)
// ═══════════════════════════════════════════════════════════════════════════

var FOB_CIF_TAB_NAME = "FOB_CIF_Calculator";

/**
 * Automatically creates and formats the 'FOB_CIF_Calculator' tab in the Google Spreadsheet.
 * Run this directly from Apps Script Editor > Run > setupFobCifCalculatorSheet
 */
function setupFobCifCalculatorSheet() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sheet = ss.getSheetByName(FOB_CIF_TAB_NAME);

  if (!sheet) {
    sheet = ss.insertSheet(FOB_CIF_TAB_NAME);
  } else {
    sheet.clear();
  }

  // Set tab color to Deep Teal
  sheet.setTabColor("#0F766E");

  // Set Column Widths
  sheet.setColumnWidth(1, 340); // Col A: Parameter / Line Item
  sheet.setColumnWidth(2, 160); // Col B: 40' High Cube
  sheet.setColumnWidth(3, 160); // Col C: 20' General Purpose
  sheet.setColumnWidth(4, 130); // Col D: Unit / Currency
  sheet.setColumnWidth(5, 280); // Col E: Notes & Formulas
  sheet.setColumnWidth(6, 120); // Col F: Matrix Col 1
  sheet.setColumnWidth(7, 120); // Col G: Matrix Col 2
  sheet.setColumnWidth(8, 140); // Col H: Matrix Col 3

  var grid = [];

  // Row 1: Header Title
  grid.push(["AMAR FOODS - EXPORT PRICE CALCULATOR (FOB / CFR / CIF)", "", "", "", ""]);
  // Row 2: Subtitle
  grid.push(["Port of Loading: Pipavav Port (Gujarat, India) | Destination: Dynamic (Hai Phong, Vietnam default) | Quotation Ref: 14-08-2026", "", "", "", ""]);
  // Row 3: Section 1 Header
  grid.push(["1. DYNAMIC GLOBAL PARAMETERS (Edit Yellow Cells to Recalculate Everything)", "", "", "", ""]);
  // Row 4: USD Rate
  grid.push(["Today's USD Exchange Rate (₹ per USD):", 88.00, 88.00, "₹ / USD", "🟡 EDIT DAILY (e.g. 88.00) - Updates all prices"]);
  // Row 5: Origin Port
  grid.push(["Origin Port of Loading:", "Pipavav Port (Gujarat)", "Pipavav Port (Gujarat)", "Port", "Fixed Origin: Pipavav Port, Gujarat, India"]);
  // Row 6: Destination Port
  grid.push(["Destination Port of Discharge:", "Hai Phong (Vietnam)", "Hai Phong (Vietnam)", "Port", "🟡 EDITABLE (e.g. Hai Phong, Jebel Ali, Rotterdam)"]);
  // Row 7: Product Name
  grid.push(["Selected Product Name:", "White Onion Flakes (Sorted)", "White Onion Flakes (Sorted)", "Grade", "A-Grade (Optical Sorted)"]);
  // Row 8: Base Cost (Ex-Factory ₹/kg)
  grid.push(["Product Base Cost (Ex-Factory Rate):", 197.00, 197.00, "₹ / kg", "🟡 EDITABLE (Base price from PriceList tab)"]);
  // Row 9: Profit Margin (₹/kg)
  grid.push(["Target Profit Margin:", 10.00, 10.00, "₹ / kg", "🟡 EDITABLE (Target export profit margin)"]);
  // Row 10: Section 2 Header
  grid.push(["2. CONTAINER SPECIFICATIONS & PAYLOAD WEIGHT", "", "", "", ""]);
  // Row 11: Container Equipment
  grid.push(["Container Equipment Type:", "40' High Cube (HC)", "20' General Purpose (GP)", "Equipment", "Food-Grade Export Containers"]);
  // Row 12: Net Weight (kg)
  grid.push(["Net Cargo Weight (Payload):", 24000, 14000, "kg", "40' HC: 24 MT (24,000 kg) | 20' GP: 14 MT (14,000 kg)"]);
  // Row 13: Section 3 Header
  grid.push(["3. LOCAL PORT LOGISTICS (PIPAVAV PORT) - AS PER 14-08-2026 QUOTATION", "", "", "", ""]);
  // Rows 14-25: 12 Itemized Logistics Line Items
  grid.push(["Transportation Pipavav - Bhadara - Pipavav (incl. 18% GST):", 18290.00, 14160.00, "₹ / Container", "At actual round-trip trucking"]);
  grid.push(["Terminal Handling Charges - THC (incl. 18% GST):", 21476.00, 13570.00, "₹ / Container", "Port terminal charges"]);
  grid.push(["CFS Handling & Stuffing Charges (incl. 18% GST):", 15340.00, 12980.00, "₹ / Container", "Container Freight Station handling"]);
  grid.push(["Buffer Yard Charges (incl. 18% GST):", 6490.00, 6490.00, "₹ / Container", "Buffer holding charges"]);
  grid.push(["BL & Export Documentation Charges (incl. 18% GST):", 6018.00, 6018.00, "₹ / Container", "Bill of Lading & export docs"]);
  grid.push(["Plant Quarantine / Phyto Certificate (incl. 18% GST):", 3304.00, 3304.00, "₹ / Container", "Phytosanitary inspection & cert"]);
  grid.push(["Fumigation Charges (Phosphine/MBr, incl. 18% GST):", 3776.00, 3776.00, "₹ / Container", "Export pest treatment"]);
  grid.push(["Agency & Forwarding Handling (incl. 18% GST):", 3304.00, 3304.00, "₹ / Container", "Forwarder handling fee"]);
  grid.push(["Container Lift On / Lift Off (Yard Pickup, incl. 18% GST):", 0.00, 2242.00, "₹ / Container", "40' HC: ₹0 at terminal | 20' GP: ₹2,242"]);
  grid.push(["Seal + MUC + EDI + VGM Charges (incl. 18% GST):", 1621.40, 1621.40, "₹ / Container", "Verified Gross Mass & customs EDI"]);
  grid.push(["Cargo Unloading Charges (incl. 18% GST):", 1274.40, 708.00, "₹ / Container", "Factory/CFS unloading"]);
  grid.push(["Weighment Charges (Pipavav Port Slip):", 600.00, 600.00, "₹ / Container", "Electronic weighbridge slip"]);
  // Row 26: Total Port Logistics (Formula)
  grid.push(["TOTAL PORT LOGISTICS (PIPAVAV PORT):", "=SUM(B14:B25)", "=SUM(C14:C25)", "₹ / Container", "40' HC: ₹81,493.80 | 20' GP: ₹68,773.40"]);
  // Row 27: Port Logistics per kg (Formula)
  grid.push(["Port Logistics Cost per kg:", "=B26/B12", "=C26/C12", "₹ / kg", "40' HC: ₹3.40/kg | 20' GP: ₹4.91/kg"]);
  // Row 28: Section 4 Header
  grid.push(["4. FOB PIPAVAV PORT CALCULATION", "", "", "", ""]);
  // Row 29: FOB Price INR
  grid.push(["FOB Pipavav Price (INR / kg):", "=B8+B9+B27", "=C8+C9+C27", "₹ / kg", "Base Cost + Margin + Port Logistics/kg"]);
  // Row 30: FOB Price USD / kg
  grid.push(["FOB Pipavav Price (USD / kg):", "=B29/$B$4", "=C29/$B$4", "$ / kg", "FOB Rate in USD per kg"]);
  // Row 31: FOB Price USD / MT
  grid.push(["FOB Pipavav Price (USD / Metric Ton):", "=B30*1000", "=C30*1000", "$ / MT", "1 Metric Ton = 1,000 kg"]);
  // Row 32: Total Container FOB Value USD
  grid.push(["TOTAL CONTAINER FOB VALUE (USD):", "=B30*B12", "=C30*C12", "$ / Container", "Total Invoice FOB Value"]);
  // Row 33: Section 5 Header
  grid.push(["5. SEA FREIGHT & CFR DESTINATION PORT CALCULATION", "", "", "", ""]);
  // Row 34: Sea Freight USD
  grid.push(["Destination Ocean Sea Freight ($ / Container):", 350.00, 225.00, "$ / Container", "🟡 EDIT FREIGHT (Hai Phong: $350 / $225)"]);
  // Row 35: Forwarder GST on Freight
  grid.push(["Forwarder GST on Sea Freight (5%):", "=B34*$B$4*0.05", "=C34*$B$4*0.05", "₹ / Container", "5% GST on ocean freight"]);
  // Row 36: Total Sea Freight INR
  grid.push(["Total Ocean Sea Freight (INR incl. 5% GST):", "=B34*$B$4*1.05", "=C34*$B$4*1.05", "₹ / Container", "Sea Freight in INR"]);
  // Row 37: Freight per kg
  grid.push(["Sea Freight Cost per kg:", "=B36/B12", "=C36/C12", "₹ / kg", "Freight impact per kg"]);
  // Row 38: CFR Price INR
  grid.push(["CFR Destination Price (INR / kg):", "=B29+B37", "=C29+C37", "₹ / kg", "FOB INR + Sea Freight INR/kg"]);
  // Row 39: CFR Price USD / kg
  grid.push(["CFR Destination Price (USD / kg):", "=B38/$B$4", "=C38/$B$4", "$ / kg", "Cost & Freight to Destination Port"]);
  // Row 40: CFR Price USD / MT
  grid.push(["CFR Destination Price (USD / Metric Ton):", "=B39*1000", "=C39*1000", "$ / MT", "1 Metric Ton = 1,000 kg"]);
  // Row 41: Total Container CFR Value USD
  grid.push(["TOTAL CONTAINER CFR VALUE (USD):", "=B39*B12", "=C39*C12", "$ / Container", "Total Invoice CFR Value"]);
  // Row 42: Section 6 Header
  grid.push(["6. MARINE CARGO INSURANCE & CIF DESTINATION PORT CALCULATION", "", "", "", ""]);
  // Row 43: Marine Insurance INR
  grid.push(["Marine Cargo Transit Insurance (incl. 18% GST):", 2950.00, 2950.00, "₹ / Container", "🟡 EDITABLE (₹2,500 + 18% GST = ₹2,950)"]);
  // Row 44: Insurance per kg
  grid.push(["Marine Insurance Cost per kg:", "=B43/B12", "=C43/C12", "₹ / kg", "Insurance impact per kg"]);
  // Row 45: CIF Price INR
  grid.push(["CIF Destination Price (INR / kg):", "=B38+B44", "=C38+C44", "₹ / kg", "CFR INR + Insurance INR/kg"]);
  // Row 46: CIF Price USD / kg
  grid.push(["CIF Destination Price (USD / kg):", "=B45/$B$4", "=C45/$B$4", "$ / kg", "🌟 FINAL CIF EXPORT QUOTE TO BUYER"]);
  // Row 47: CIF Price USD / MT
  grid.push(["CIF Destination Price (USD / Metric Ton):", "=B46*1000", "=C46*1000", "$ / MT", "1 Metric Ton = 1,000 kg"]);
  // Row 48: Total Container CIF Value USD
  grid.push(["TOTAL CONTAINER CIF VALUE (USD):", "=B46*B12", "=C46*C12", "$ / Container", "🌟 TOTAL BUYER CONTRACT VALUE ($)"]);
  // Row 49: Empty spacer
  grid.push(["", "", "", "", ""]);
  // Row 50: Section 7 Header
  grid.push(["7. GLOBAL OCEAN FREIGHT PRESETS (From Pipavav Port)", "", "", "", ""]);
  // Rows 51-57: Global Ports Table
  grid.push(["Destination Port & Country", "40' HC Freight ($)", "20' GP Freight ($)", "Transit Days (Est)", "Notes / Trade Route"]);
  grid.push(["Hai Phong, Vietnam (Official Quote)", 350, 225, "18 - 22 Days", "Direct / Feeder via Singapore"]);
  grid.push(["Jebel Ali / Dubai, UAE", 450, 300, "5 - 7 Days", "Major Middle East Hub"]);
  grid.push(["Dammam / Jeddah, Saudi Arabia", 600, 400, "8 - 12 Days", "Gulf Region"]);
  grid.push(["Felixstowe / Rotterdam / Hamburg, Europe", 1650, 1100, "25 - 32 Days", "Western Europe"]);
  ...
  sheet.getRange("A2:E2").merge()
    .setBackground("#E6FFFA")
    .setFontColor("#0D9488")
    .setFontStyle("italic")
    .setFontSize(10)
    .setHorizontalAlignment("center");

  // Section Headers: Rows 3, 10, 13, 28, 33, 42, 50
  var sectionRows = [3, 10, 13, 28, 33, 42, 50];
  ...
  // Row 11: Column Headers for 40' HC and 20' GP
  sheet.getRange("B11:C11")
    .setBackground("#0F766E")
    .setFontColor("#FFFFFF")
    .setFontWeight("bold")
    .setHorizontalAlignment("center");

  // Highlight Editable Yellow Cells
  var yellowCells = ["B4", "C4", "B6", "C6", "B8", "C8", "B9", "C9", "B34", "C34", "B43", "C43"];
  for (var y = 0; y < yellowCells.length; y++) {
    sheet.getRange(yellowCells[y])
      .setBackground("#FEF3C7")
      .setFontWeight("bold")
      .setBorder(true, true, true, true, false, false, "#F59E0B", SpreadsheetApp.BorderStyle.SOLID_MEDIUM);
  }

  // Row 26: Total Logistics Row
  sheet.getRange("A26:E26")
    .setBackground("#F1F5F9")
    .setFontWeight("bold");

  // Key Calculation Rows Styling
  // FOB USD: Row 30 & 32
  sheet.getRange("A30:D30").setBackground("#DCFCE7").setFontWeight("bold");
  sheet.getRange("A32:D32").setBackground("#D1FAE5").setFontWeight("bold").setFontSize(11);

  // CFR USD: Row 39 & 41
  sheet.getRange("A39:D39").setBackground("#FEF9C3").setFontWeight("bold");
  sheet.getRange("A41:D41").setBackground("#FEF08A").setFontWeight("bold").setFontSize(11);

  // CIF USD: Row 46 & 48 (Golden / Green Star Rows)
  sheet.getRange("A46:D46").setBackground("#BBF7D0").setFontWeight("bold").setFontSize(12);
  sheet.getRange("A48:D48").setBackground("#FDE047").setFontWeight("bold").setFontSize(12);

  // Presets Table Headers (Row 51)
  sheet.getRange("A51:E51")
  ...
  sheet.getRange("B8:C9").setNumberFormat("₹#,##0.00");
  sheet.getRange("B12:C12").setNumberFormat("#,##0");
  sheet.getRange("B14:C26").setNumberFormat("₹#,##0.00");
  sheet.getRange("B27:C27").setNumberFormat("₹#,##0.00");
  sheet.getRange("B29:C29").setNumberFormat("₹#,##0.00");
  sheet.getRange("B30:C30").setNumberFormat("$#,##0.000");
  sheet.getRange("B31:C31").setNumberFormat("$#,##0.00");
  sheet.getRange("B32:C32").setNumberFormat("$#,##0.00");
  sheet.getRange("B34:C34").setNumberFormat("$#,##0.00");
  sheet.getRange("B35:C37").setNumberFormat("₹#,##0.00");
  sheet.getRange("B38:C38").setNumberFormat("₹#,##0.00");
  sheet.getRange("B39:C39").setNumberFormat("$#,##0.000");
  sheet.getRange("B40:C40").setNumberFormat("$#,##0.00");
  sheet.getRange("B41:C41").setNumberFormat("$#,##0.00");
  sheet.getRange("B43:C45").setNumberFormat("₹#,##0.00");
  sheet.getRange("B46:C46").setNumberFormat("$#,##0.000");
  sheet.getRange("B47:C47").setNumberFormat("$#,##0.00");
  ...
  // Populate 24 Baseline Products dynamically linked to PriceList
  var baselineProducts = getBaseline24Products(Utilities.formatDate(new Date(), "Asia/Kolkata", "dd-MMM-yyyy"));
  var matrixRows = [];

  for (var p = 0; p < baselineProducts.length; p++) {
    var pItem = baselineProducts[p];
    var code = pItem[0];
    var pName = pItem[2];
    var pGrade = pItem[3];
    var defaultInr = pItem[6]; // e.g. 197
    var matrixRowIndex = 62 + p;

    // Formulas:
    // Ex-Factory INR: If PriceList exists, formula: =IFERROR(VLOOKUP(A${matrixRowIndex}, PriceList!A:G, 7, FALSE), ${defaultInr})
    var inrFormula = '=IFERROR(VLOOKUP(A' + matrixRowIndex + ', PriceList!A:G, 7, FALSE), ' + defaultInr + ')';
    // FOB Pipavav $/kg: =(D${matrixRowIndex} + $B$9 + $B$27) / $B$4
    var fobFormula = '=(D' + matrixRowIndex + ' + $B$9 + $B$27) / $B$4';
    // CFR Hai Phong $/kg: =E${matrixRowIndex} + ($B$37 / $B$4)
    var cfrFormula = '=E' + matrixRowIndex + ' + ($B$37 / $B$4)';
    // CIF Hai Phong $/kg: =F${matrixRowIndex} + ($B$44 / $B$4)
    var cifFormula = '=F' + matrixRowIndex + ' + ($B$44 / $B$4)';
    // 40' HC Container CIF Total ($): =G${matrixRowIndex} * $B$12
    var total40Formula = '=G' + matrixRowIndex + ' * $B$12';
  ...
  sheet.getRange(62, 1, matrixRows.length, 8).setValues(matrixRows);

  // Format Matrix Columns
  sheet.getRange(62, 4, matrixRows.length, 1).setNumberFormat("₹#,##0.00");
  sheet.getRange(62, 5, matrixRows.length, 3).setNumberFormat("$#,##0.000");
  sheet.getRange(62, 8, matrixRows.length, 1).setNumberFormat("$#,##0.00");

  // Alternating row colors for Matrix
  for (var mr = 62; mr < 62 + matrixRows.length; mr++) {
    var bg = (mr % 2 === 0) ? "#F8FAFC" : "#FFFFFF";
    sheet.getRange(mr, 1, 1, 8).setBackground(bg);
  }

  // Freeze top 3 rows
  sheet.setFrozenRows(3);

  Logger.log("✅ FOB_CIF_Calculator tab created and configured successfully!");
  return {
    status: "success",
    sheetName: FOB_CIF_TAB_NAME,
    message: "FOB & CIF Calculator sheet configured with 12 Pipavav port line items and 24-product matrix."
  };
}

/**
 * Returns the current parameters and calculated rates from the FOB_CIF_Calculator tab.
 */
function getFobCifCalculatorData() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sheet = ss.getSheetByName(FOB_CIF_TAB_NAME);
  if (!sheet) {
    setupFobCifCalculatorSheet();
    sheet = ss.getSheetByName(FOB_CIF_TAB_NAME);
  }

  var usdRate = Number(sheet.getRange("B4").getValue()) || 88.00;
  var originPort = String(sheet.getRange("B5").getValue()) || "Pipavav Port (Gujarat)";
  var destPort = String(sheet.getRange("B6").getValue()) || "Hai Phong (Vietnam)";
  var baseCost = Number(sheet.getRange("B8").getValue()) || 197.00;
  var margin = Number(sheet.getRange("B9").getValue()) || 10.00;

  var freight40 = Number(sheet.getRange("B34").getValue()) || 350.00;
  var freight20 = Number(sheet.getRange("C34").getValue()) || 225.00;
  var insurance = Number(sheet.getRange("B43").getValue()) || 2950.00;

  var fobInr40 = Number(sheet.getRange("B29").getValue()) || 0;
  var fobUsd40 = Number(sheet.getRange("B30").getValue()) || 0;
  var cfrUsd40 = Number(sheet.getRange("B39").getValue()) || 0;
  var cifUsd40 = Number(sheet.getRange("B46").getValue()) || 0;
  var totalCif40 = Number(sheet.getRange("B48").getValue()) || 0;

  var fobInr20 = Number(sheet.getRange("C29").getValue()) || 0;
  var fobUsd20 = Number(sheet.getRange("C30").getValue()) || 0;
  var cfrUsd20 = Number(sheet.getRange("C39").getValue()) || 0;
  var cifUsd20 = Number(sheet.getRange("C46").getValue()) || 0;
  var totalCif20 = Number(sheet.getRange("C48").getValue()) || 0;
  ...
  return {
    ...
    insurance: insurance,
    summary40: {
      logisticsTotalInr: Number(sheet.getRange("B26").getValue()) || 81493.80,
      logisticsPerKgInr: Number(sheet.getRange("B27").getValue()) || 3.40,
      fobInr: fobInr40,
      fobUsdKg: fobUsd40,
      cfrUsdKg: cfrUsd40,
      cifUsdKg: cifUsd40,
      cifTotalUsd: totalCif40
    },
    summary20: {
      logisticsTotalInr: Number(sheet.getRange("C26").getValue()) || 68773.40,
      logisticsPerKgInr: Number(sheet.getRange("C27").getValue()) || 4.91,
      fobInr: fobInr20,
      fobUsdKg: fobUsd20,
      cfrUsdKg: cfrUsd20,
      cifUsdKg: cifUsd20,
      cifTotalUsd: totalCif20
    }
  };
}

/**
 * Updates dynamic inputs in the FOB_CIF_Calculator tab (USD Rate, Destination Port, Freight, etc.)
 */
function updateFobCifInputs(params) {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sheet = ss.getSheetByName(FOB_CIF_TAB_NAME);
  if (!sheet) {
    setupFobCifCalculatorSheet();
  ...
  if (params.usdRate) {
    var rate = Number(params.usdRate);
    if (!isNaN(rate) && rate > 0) {
      sheet.getRange("B4:C4").setValue(rate);
    }
  }

  if (params.destinationPort) {
    sheet.getRange("B6:C6").setValue(String(params.destinationPort));
  }

  if (params.baseCost) {
    var cost = Number(params.baseCost);
    if (!isNaN(cost) && cost > 0) {
      sheet.getRange("B8:C8").setValue(cost);
    }
  }

  if (params.margin !== undefined) {
    var m = Number(params.margin);
    if (!isNaN(m)) {
      sheet.getRange("B9:C9").setValue(m);
    }
  }

  if (params.freight40) {
    var f40 = Number(params.freight40);
    if (!isNaN(f40) && f40 >= 0) sheet.getRange("B34").setValue(f40);
  }

  if (params.freight20) {
    var f20 = Number(params.freight20);
    if (!isNaN(f20) && f20 >= 0) sheet.getRange("C34").setValue(f20);
  }

  if (params.insurance) {
    var ins = Number(params.insurance);
    if (!isNaN(ins) && ins >= 0) sheet.getRange("B43:C43").setValue(ins);
  }

  SpreadsheetApp.flush();
  return getFobCifCalculatorData();
}
